...
/** معرّف المتجر في المسار هو `places.id` (توافق مع العميل). */
...
router.get('/', async (req, res, next) => {
  try {
    const { rows } = await pool.query(
      `SELECT * FROM products WHERE place_id = $1 AND is_available = true
       ORDER BY sort_order, name`,
      [req.params.storeId]
    );
    return success(res, rows.map(rowToProduct));
  } catch (err) {
    next(err);
  }
});
  ...
  try {
    ...
    const { rows } = await pool.query(
      `INSERT INTO products (
         place_id, name, description, price, image_url, stock,
         main_category, sub_category, company_name
       )
    ...
    return success(res, rowToProduct(rows[0]), 201);
  } catch (err) {
    next(err);
  }
});
...
router.patch('/:id', authenticate, async (req, res, next) => {
  try {
    ...
    if (main_category !== undefined) {
      updates.push(`main_category = $${i++}`);
      values.push(main_category);
    }
    if (sub_category !== undefined) {
      updates.push(`sub_category = $${i++}`);
    ...
    return success(res, rowToProduct(rows[0]));
  } catch (err) {
    next(err);
  }
});
...
router.delete('/:id', authenticate, async (req, res, next) => {
  try {
    ...
    const { rowCount } = await pool.query(
      'DELETE FROM products WHERE id = $1 AND place_id = $2',
