...
### ابدأ من هنا
- [نسخة واضحة وسريعة (Recommended)](#نسخة-واضحة-وسريعة-recommended)
...
- [شرح كل ملف (للمبتدئ)](#شرح-كل-ملف-للمبتدئ)
- [سير عمل إضافة مكان](#سير-عمل-إضافة-مكان)
- [مصادر إضافية](#مصادر-إضافية)

---

## نسخة واضحة وسريعة (Recommended)

إذا كنت تريد تشغيل المشروع بسرعة وبدون قراءة كل التفاصيل، اتبع هذا القسم أولاً.

### المشروع باختصار
- **الواجهة:** Expo + React Native + TypeScript (موبايل + ويب)
- **الخادم:** Node.js + Express
- **قاعدة البيانات:** PostgreSQL
- **المصادقة:** JWT (Access + Refresh)
- **رفع الصور:** Cloudinary

### التشغيل في 5 خطوات فقط

1. من جذر المشروع ثبّت الواجهة:
   ```bash
   npm install
   ```
2. ثبّت الخادم:
   ...
   ```
3. أنشئ ملفات البيئة:
   - `server/.env` من `server/.env.example`
   - `.env` في الجذر من `.env.example`
4. شغّل الهجرة:
   ```bash
   cd server
   npm run migrate:v1
...
**في `.env` (جذر المشروع):**
- `EXPO_PUBLIC_API_URL` (مثال: `http://localhost:3000`)
- `EXPO_PUBLIC_USE_API=true`
- `EXPO_PUBLIC_GOOGLE_MAPS_API_KEY`

### كيف تقرأ هذا الملف؟
- للتشغيل فقط: اقرأ **البدء السريع** + **إعداد الخادم**.
- لفهم الكود: ابدأ من **نظرة عامة على المشروع** ثم **هيكل الـ API**.
- لفهم قاعدة البيانات: اقرأ **مخطط قاعدة البيانات** ثم **هيكل قاعدة البيانات**.
- إذا كنت جديداً على المشروع: اقرأ **شرح كل ملف (للمبتدئ)**.
